...
function RotationCard({ rotation }) {
  ...
  return (
    <Card>
      <SectionLabel>{t("lol.rotation")}</SectionLabel>
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        {champs.map(c => (
          <View key={c.id} style={styles.champ}>
            <Image source={{ uri: championIcon(c.name) }} style={styles.img} />
            <Text style={styles.name} numberOfLines={1}>{championLabel(c.name)}</Text>
          </View>
        ))}
      </ScrollView>
    </Card>
...
export default function LolHomeExtras({ region }) {
  const { language } = useI18n();   // el estado del servidor viene en el idioma activo: se vuelve a pedir al cambiarlo
