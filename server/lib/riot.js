...
function toHttpError(e, notFound) {
  const provider = "Riot";
  const notFoundMessage = notFound?.message ?? notFound;
  const notFoundCode = notFound?.code || "NOT_FOUND";
  ...
  if (status === 404) return new HttpError(404, notFoundMessage || "No se encontró lo que buscabas", { code: notFoundCode });
  if (status === 400) return new HttpError(400, "Solicitud inválida", { code: "BAD_REQUEST" });
  if (status === 401 || status === 403) {
    ...
    return new HttpError(503, "Riot rechazó la consulta: la key no es válida o no tiene acceso a esta API todavía", { code: "KEY_INVALID", provider });
  }
  if (status === 429) {
    const retryAfter = parseInt(e.response.headers?.["retry-after"], 10) || 5;
    return new HttpError(429, `Riot limitó las solicitudes, reintenta en ${retryAfter} s`, { retryAfter, code: "RATE_LIMITED", provider });
  }
  if (e.code === "ECONNABORTED" || e.code === "ETIMEDOUT") return new HttpError(504, "Riot tardó demasiado en responder", { code: "TIMEOUT", provider });
  if (status >= 500) return new HttpError(502, "Riot tuvo un problema, intenta de nuevo", { code: "UPSTREAM_ERROR", provider });
  return new HttpError(502, "No se pudo contactar a Riot", { code: "UPSTREAM_UNREACHABLE", provider });
}

/**
 * GET a Riot con caché, deduplicación y errores normalizados.
 * opts.ttl: milisegundos de caché (0 = sin caché); opts.notFound: mensaje para 404, o { message, code }.
 */
...
function errorHandler(err, req, res, _next) {
  const e = err instanceof HttpError ? err : new HttpError(500, "Error interno del servidor", { code: "INTERNAL" });
  ...
  res.status(e.status).json({ error: e.message, code: e.code, provider: e.provider, retryAfter: e.retryAfter });
}
