...
- **Sonido, ícono e imagen de la notificación (Android):** dos canales con sonido propio (`live_start` y `live_result`, WAV generados con `scripts/generate-notification-sounds.py`), ícono pequeño en forma de escudo con la K en hueco (`scripts/generate-notification-icon.py`) y, al expandirla, una imagen ancha con el splash del campeón que el servidor compone en `/art/banner.png`. Las URLs van firmadas (`ART_SECRET`, opcional) y usan la dirección pública del servidor (`PUBLIC_BASE_URL` o `RENDER_EXTERNAL_URL`). Si el motor de imágenes no carga, las notificaciones salen sin imagen. Expo Push no permite un ícono grande, por eso no lo lleva.
